...
export class YtplayerComponent
		...
		try {
			...
			this.player = new window['YT'].Player(container, {
				...
				playerVars: {
					...
					playsinline: 1,
					rel: 0,
					origin: "https://youtube-proxy-7jh9gm4hz-mrridsyss-projects.vercel.app",
					showinfo: 0,
					iv_load_policy: 3,
					fs: 0,
					cc_load_policy: 0
	...
	private handleReady(event: YT.PlayerEvent) {
		...
		try {
			...
			// Step 1: Always start muted (autoplay allowed everywhere)
			if (target.mute) target.mute();
			if (target.playVideo) target.playVideo();

			//  Step 2: Unmute after a small delay (optional)
			// Some TVs block this — but browsers will allow it silently.
			setTimeout(() => {
				try {
					if (target && target.unMute) target.unMute();
				} catch (e) {
